...
plotEdge <- function(x1, x2, subtree, ePar, horiz = FALSE, gratio, max.level, group, cex, nc, cpal) {
	...
	if (type == "rectangle") {
		...
		if (horiz) {			
			segments(xTop, yTop, xTop+0.5, yTop, col=col, lty=lty, lwd=lwd)
		} else {
			segments(X, yTop, X, yTop+0.5, col=col, lty=lty, lwd=lwd)
		}
	}
		...
		idx <- which(k==children)
			...
			if (type == "triangle") {
				...
				if (horiz) {
					...
					segments(xTop+0.5, xBot, xTop+1, xBot, col=ecol, lty=lty, lwd=lwd)
				} else {
					segments(xTop, yTop+0.5, xBot, yTop+0.5, col=col, lty=lty, lwd=lwd)

					## Edge from the node to edge from parent
                	segments(xBot, yTop+0.5, xBot, yTop+1, col=ecol, lty=lty, lwd=lwd)
